...
- **Tipo de imagem:** duas fotos distintas do médico, do mesmo ensaio profissional (Canon EOS R6m2, fundo warm-tostado). **Hero:** retrato formal (terno, meio corpo, braços cruzados no peito, sério). **Autoridade:** retrato próximo (camisa social azul, meio corpo, braços cruzados, sorriso aberto) — dois registros diferentes cumprem `docs/uso-de-fotos.md` (a regra é sobre não repetir a **mesma** foto, não sobre não ter duas fotos do médico). Sem stock. Sem antes/depois. Sem paciente identificável.
...
| Autoridade | Formação + segundo registro visual do médico | `ref.visual.forward.saude-humana-clara` (elementos: foto real, CRM/RQE em subtitulo, sem infantilizar) | Two-column: retrato próximo do médico + bloco de credenciais em lista | Retrato próximo/informal do médico (camisa social, sorrindo — 139A6271, ensaio distinto do hero), lista de credenciais em card, CRM/RQE em destaque. Regra `docs/uso-de-fotos.md` é sobre não repetir a **mesma** foto, não sobre não ter duas fotos do médico | Foto de banco quebra tudo (spec saude); reusar a MESMA foto do hero aqui cansa e enfraquece — foto distinta do mesmo ensaio é prova adicional legítima |
